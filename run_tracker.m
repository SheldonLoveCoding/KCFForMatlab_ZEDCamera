...
function [precision, fps] = run_tracker(video, kernel_type, feature_type, show_visualization, show_plots)
	...
	base_path = 'C:/Users/root/Desktop/aba/tracker_myrelease/data/Benchmark/';
    ...
    case 'Camera',
        ...
        img_first = snapshot(zed);
        img_first_left = img_first(:, 1 : width/2, :);
        figure;imshow(img_first_left);hold on;
        title('Please label the top_left point and the bottom_right point of the object! ! !');
        rec = getrect %[xmin ymin width height]
        rectangle('Position',rec,'LineWidth',4,'EdgeColor','r');
        target_sz = [rec(4) rec(3)];
        pos = [rec(2) rec(1)] + floor(target_sz/2);
        
        [positions, time] = tracker2(width, zed, pos, target_sz, ...
			padding, kernel, lambda, output_sigma_factor, interp_factor, ...
			cell_size, features);
        save('pos.mat','positions');
        % 先随便给一个数
        precision = 0;
        fps = 0;
	otherwise
